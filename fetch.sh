...
if [ "$yes" = "1" ]; then
  for f in $(ls secrets/*); do
    ...
    if [ -f $target ]; then
      ...
      curl --cookie session=$(cat $f) --user-agent 'https://github.com/petertseng/adventofcode-hs-2025/blob/master/fetch.sh' -o $target https://adventofcode.com/2025/day/$day/input
      sleep 1
    fi
  done
else
